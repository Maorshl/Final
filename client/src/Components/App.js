...
import OnePostById from "./OnePostById";
...
async function getNewToken(refToken) {
  try {
    const token = await axios.post(
      "http://ec2-3-80-252-156.compute-1.amazonaws.com:8080/user/refreshToken",
      {
        refToken: refToken,
      }
    );
      ...
      <Router>
        <Switch>
          ...
          <Route exact path="/post/:id">
            {user ? <OnePostById setUser={setUser} /> : <NeedToLogin />}
          </Route>
          <Route exact path="/editPost/:id">
            {user ? <EditPost setUser={setUser} /> : <NeedToLogin />}
          </Route>
          <Route path="/">
