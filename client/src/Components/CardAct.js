...
function CardAct({ post }) {
  ...
  useEffect(() => {
    ...
    const getRaters = async () => {
      const { data } = await axios.get(
        `http://app.smartlibrary.link:8080/rating/isRated?id=${post._id}&userName=${userName}`
      );
      setIsRated(data);
    };
    ...
    const getAVGRateData = async () => {
      const { data } = await axios.get(
        `http://app.smartlibrary.link:8080/rating/getRate?id=${post._id}`
      );
  ...
  const setPostRate = async (rate) => {
    const { data } = await axios.post(
      "http://app.smartlibrary.link:8080/rating/ratePost",
      {
        postId: post._id,
        userName,
        rate,
  ...
  async function likeThePost(postId) {
    await axios.post("http://app.smartlibrary.link:8080/user/save", {
      userName,
      postId,
    });
    setLiked(true);
  ...
  async function unlikeThePost(postId) {
    await axios.patch(
      "http://app.smartlibrary.link:8080/user/removeFromSaved",
      {
        userName,
        postId,
      }
